  ...
  return async (dispatch: Dispatch) => {
    DatabaseService.getAllRecords("plugins").then(async (pluginList) => {
      try {
        ...
        if (ConfigService.getReaderConfig("aiWikiModel")) {
          const modelKey = ConfigService.getReaderConfig("aiWikiModel");
          const entry = ConfigService.getObjectConfig(
            modelKey,
            "aiModelConfig",
            null
          ...
          if (entry && entry.key) {
            let wikiPlugin = new PluginModel(
              "custom-ai-wiki-plugin",
              "wiki",
              "Custom AI Wiki",
              "assistant",
              "1.0.0",
