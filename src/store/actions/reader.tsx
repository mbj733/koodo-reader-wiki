...
import { WIKI_DB_NAME } from "../../utils/wiki/wikiService";
export function handleWikis(wikis: WikiModel[]) {
  return { type: "HANDLE_WIKIS", payload: wikis };
}
export function handleFetchWikis() {
  return async (dispatch: any, getState: any) => {
    const state = getState();
    const currentBook = state.book.currentBook;
    if (!currentBook || !currentBook.key) return;
    try {
      let wikis = await DatabaseService.getRecordsByBookKey(
        currentBook.key,
        WIKI_DB_NAME
      );
      dispatch(handleWikis(wikis || []));
    } catch (e) {
      console.error("handleFetchWikis error:", e);
      dispatch(handleWikis([]));
    }
  };
}
export function handleSaveWiki(wiki: WikiModel) {
  return async (dispatch: any) => {
    try {
      await DatabaseService.saveRecord(wiki, WIKI_DB_NAME);
      dispatch(handleFetchWikis() as any);
    } catch (e) {
      console.error("handleSaveWiki error:", e);
    }
  };
}
export function handleDeleteWiki(key: string) {
  return async (dispatch: any) => {
    try {
      await DatabaseService.deleteRecord(key, WIKI_DB_NAME);
      dispatch(handleFetchWikis() as any);
    } catch (e) {
      console.error("handleDeleteWiki error:", e);
    }
  };
}
