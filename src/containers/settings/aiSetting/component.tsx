...
class AISetting extends React.Component<SettingInfoProps, SettingInfoState> {
  constructor(props: SettingInfoProps) {
    ...
    this.state = {
      ...
      aiWikiModel:
        ConfigService.getReaderConfig("aiWikiModel") || "",
      aiWikiPrompt: ConfigService.getReaderConfig("aiWikiPrompt") || "",
      aiTranslatePrompt:
  ...
  handleDelete = (key: string) => {
    try {
      ...
      if (this.state.aiWikiModel === key) {
        this.setState({ aiWikiModel: "" });
        ConfigService.setReaderConfig("aiWikiModel", "");
      }
  ...
  handleEditPrompt = async (
    type: "aiTranslate" | "aiDict" | "aiAssistance" | "aiWiki"
  ) => {
  ...
  render() {
    ...
    return (
      <>
        ...
        <div className="setting-dialog-new-title">
          <span style={{ display: "flex", alignItems: "center", gap: "8px" }}>
            <Trans>AI wiki model</Trans>
            <span
              className="change-location-button"
              style={{ fontSize: "12px" }}
              onClick={() => this.handleEditPrompt("aiWiki")}
            >
              <Trans>Edit prompt</Trans>
            </span>
          </span>
          <select
            className="lang-setting-dropdown"
            value={this.state.aiWikiModel}
            onChange={(e) => {
              const val = e.target.value;
              this.setState({ aiWikiModel: val });
              ConfigService.setReaderConfig("aiWikiModel", val);
              toast.success(this.props.t("Change successful"));
              this.props.handleFetchPlugins();
            }}
          >
